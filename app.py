...
def frequency_signature(data):
    ...
    return round(sum(ascii_vals) / len(ascii_vals), 4)
...
def loginback():
    if request.method == "POST":
        ...
        # 2) Then check if any user found
        if len(results) == 0:
            flash("Login failed", "warning")
            return render_template('tender.html', msg="Login Failure!!!")

        # 3) Now it's safe to access results[0]
        global name
        name = results[0][1]
        print(name)
...
def maketenderback():
    if request.method == 'POST':
        ...
        # 🔌 ECE SIGNAL ANALYSIS
        # =========================
        entropy = calculate_signal_entropy(data)
        freq_sig = frequency_signature(data)
        signal_fingerprint = f"E={entropy},F={freq_sig}"
        ...
        (tid,temail,obj,cost,sdate,edate,email,pan,adhar,file,
        hash1,hash2,date,time1,entropy,freq_sig,signal_fingerprint)
        VALUES
        (%s,%s,%s,%s,%s,%s,%s,%s,%s,
        AES_ENCRYPT(%s,'lakshmi'),
        %s,%s,%s,%s,%s,%s,%s)
        """
        ...
        hash1, hash2,
        currentDay, t1,
        entropy, freq_sig, signal_fingerprint
        )
...
def viewbid():
    ...
    if today > yesterday:
        ...
        # ❌ DO NOT drop display columns
        # ✅ Only drop sensitive / technical columns
        drop_cols = [
            'file', 'hash1', 'hash2',
            'pan', 'adhar'
        ]
...
def down():
    if request.method == 'POST':
        ...
        sql = "SELECT AES_DECRYPT(file,'lakshmi') FROM tender_files WHERE id=%s"
        ...
        # 🔁 Recalculate hashes
        mid = len(data) // 2
        new_hash1 = hashlib.sha1(data[:mid].encode()).hexdigest()
        new_hash2 = hashlib.sha1(data[mid:].encode()).hexdigest()

        if new_hash1 != hash1 or new_hash2 != hash2:
            flash("File verification failed (Tampered Data)", "danger")
            return render_template(
                "tampered.html",
                old1=hash1,
                new1=new_hash1,
        ...
        return render_template("downfile.html", msg=data)
...
def finalised(s=0, s1=''):
    ...
    if count == 1:
        ...
        t1 = 'Tender Officer.'
        mail_content = msg + '\n' + otp + '\n' + '\n' + t + '\n' + t1
        sender_address = 'yella.anuhya21@gmail.com'
        sender_pass = 'bdge ciie rqdt culn'
        receiver_address = s1
